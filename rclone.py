        ...
        for crypt_dir, mapped_remotes in self._crypt_mappings.items():
            ...
            if path.lower().startswith(crypt_dir.lower()):
                for mapped_remote in mapped_remotes:
                    ...
                    logger.debug("Crypt base directory '%s' has mapping defined in config as remote '%s'.", crypt_dir,
                                 mapped_remote)
                    logger.info("Attempting to decode...")
                    logger.debug("Raw query is: '%s'", " ".join(
                ...
                logger.debug("Ignoring crypt decode for path '%s' because '%s' was not matched from 'CRYPT_MAPPINGS'.",
                             path, crypt_dir)
        return None
